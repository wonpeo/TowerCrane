...
import { JobOfferPostingsApi, JobOfferPostingsByCareerApi, JobOfferPostingsByJobClassApi, JobOfferPostingsByLocationApi } from '../../../apis';
...
export default function DataTable() {
    ...
    useEffect(() => {
      const fetchData = async () => {
          const data = await JobOfferPostingsApi();
          setRows(data);
  ...
  useEffect(() => {
      ...
      if (filterJobClass.length > 0) {
          data = data.filter(item => filterJobClass.includes(item.requirementJobClass));
      }

      if (filterCareer.length > 0) {
          data = data.filter(item => item.requiredCareer >= Number(filterCareer[0]));
      }

      if (filterLocation.length > 0) {
          data = data.filter(item => filterLocation.includes(item.location));
      }
      ...
      setFilteredRows(data);
  }, [filterJobClass, filterCareer, filterLocation, sortType, rows]);
...
<MenuItem key={index} value={`${career}`}>{`${career}년 이상`}</MenuItem> 
)}
</Select>
</FormControl>
