...
import { jobOffererSignInApi } from '../../../apis'; 

export default function JobOffererSignIn() {
...
const signInHandler = async () => {
    ...
    try{
      let signInResponse = await jobOffererSignInApi(data);
      if (signInResponse && signInResponse.result === true) {
        console.log(signInResponse.data.jobOfferer);
        alert("Welcome");
         // 성공 후 필드 초기화
         resetFields();
         const user = signInResponse.data.jobOfferer;
         localStorage.setItem('username', user.joName);
         localStorage.setItem('joid', user.joId);  
         localStorage.setItem('companyname', user.companyName);  

       localStorage.setItem('isLoggedIn', 'true');
       localStorage.setItem('isLoggedInJo', 'true');
  ...
  function resetFields() {
   ...
   return (
    <Card sx={{ width:"500px",margin:"auto",marginTop:"100px"}}>
        <Box>
           <Typography sx={{textAlign:"center",marginTop:"50px",fontFamily:"unset"}}variant='h5'>기업 로그인</Typography>
